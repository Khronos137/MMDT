...
def image_callback_color(img_msg):
...
def image_callback_depth(img_msg):
    # print('Recibi imagen')
    global img_depth
    global tamW_depth
    global tamH_depth
...
if __name__ == '__main__':
    try:
        ...
        # Initalize a subscriber to the "/camera/rgb/image_raw" topic with the function "image_callback" as a callback
        depth_image = rospy.Subscriber("/3D_camera/depth/image_raw", Image, image_callback_depth, queue_size=1)
        color_image = rospy.Subscriber("/mmdt_rat20/cameraTop/image_raw", Image, image_callback_color, queue_size=1)
